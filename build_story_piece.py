...
<link href="https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,400;0,9..144,600;0,9..144,700;0,9..144,800;1,9..144,400;1,9..144,600&family=JetBrains+Mono:wght@400;500;600;700&family=Newsreader:ital,opsz,wght@0,6..72,400;0,6..72,500;0,6..72,600;1,400;1,600&family=Plus+Jakarta+Sans:ital,wght@0,400;0,500;0,600;0,700;1,400;1,600&display=swap" rel="stylesheet">
...
.step-card {{
  ...
  border: 1px solid rgba(224, 218, 209, 0.85);
  border-radius: 14px;
  padding: 2rem 2.2rem;
  box-shadow: 0 10px 30px rgba(45, 64, 89, 0.05), 0 1px 3px rgba(0, 0, 0, 0.03);
  width: 100%;
  max-width: 460px;
...
.step.is-active .step-card {{
  ...
  box-shadow: 0 16px 40px rgba(45, 64, 89, 0.09), 0 2px 8px rgba(200, 75, 49, 0.05);
  transform: translateY(-2px);
}}
.step.is-active .step-card::before {{
  content: '';
  position: absolute;
  top: 0;
  left: 0;
  ...
  <div class="scrolly-layout">
    ...
    <div class="scrolly-steps">
      ...
      <div class="step" data-step="1">
        <div class="step-card">
          <div class="step-tag"><span class="tag-bullet"></span>Chamber Attendance</div>
      ...
      <div class="step" data-step="2">
        <div class="step-card">
          <div class="step-tag"><span class="tag-bullet"></span>Written Scrutiny</div>
      ...
      <div class="step" data-step="3">
        <div class="step-card">
          <div class="step-tag"><span class="tag-bullet"></span>Celebrities & Workhorses</div>
      ...
      <div class="step" data-step="4">
        <div class="step-card">
          <div class="step-tag"><span class="tag-bullet"></span>State Patterns</div>
      ...
      <div class="step" data-step="5">
        <div class="step-card">
          <div class="step-tag"><span class="tag-bullet"></span>Gender Focus</div>
      ...
      <div class="step" data-step="6">
        <div class="step-card">
          <div class="step-tag"><span class="tag-bullet"></span>National Priorities</div>
          <h3 class="step-title">The National Wishlist</h3>
          <div class="step-body">
            <p>What topics did MPs ask about most? Questions centered heavily on basic welfare and citizen priorities: <strong>Health & Family Welfare (7,595), Agriculture (5,122), Education (3,589), and Environment & Climate (3,535)</strong> led the docket.</p>
            <p>Sustained scrutiny also focused on <strong>Women & Child Development (3,063), Labour & Employment (2,688), and Rural Development (2,545)</strong> across all parties.</p>
            <p>In contrast, technical departments received almost no questions: the <strong>Department of Space received just 1 question</strong> in five years, and Atomic Energy received 8.</p>
          </div>
          <div class="step-annotation">
            🔍 <strong>Welfare Focus:</strong> Public healthcare, farming, schools, and livelihoods make up the overwhelming majority of floor questions.
          </div>
        </div>
      </div>
...
function setupStep1() {{
  document.getElementById('hud-step-title').textContent = 'Chamber Overview · 17th Lok Sabha';
...
function setupStep2() {{
  document.getElementById('hud-step-title').textContent = 'Attendance vs. Questions Asked';
  gGrid.selectAll('*').remove();
  gAnnotations.selectAll('*').remove();
  gAxes.selectAll('*').remove();
...
function setupStep3() {{
  document.getElementById('hud-step-title').textContent = 'Celebrity Backbenchers vs. Legislative Workhorses';
  setupStep2();
...
function setupStep4() {{
  document.getElementById('hud-step-title').textContent = 'Questions by State · Maharashtra Pattern';
  gGrid.selectAll('*').remove();
  gAnnotations.selectAll('*').remove();
  gAxes.selectAll('*').remove();
...
// Step 5: Gender Lens
function setupStep5() {{
  document.getElementById('hud-step-title').textContent = 'Questions by Gender · Female vs. Male Priorities';
  setupStep2();
...
function setupStep6() {{
  document.getElementById('hud-step-title').textContent = 'The National Wishlist · Questions by Sector';
  gGrid.selectAll('*').remove();
  gAnnotations.selectAll('*').remove();
  gAxes.selectAll('*').remove();

  // Ministries with balanced coordinates and clear color coding
  const clusters = {{
    'Health and Family Welfare': {{ x: width * 0.22, y: height * 0.25, color: '#DC2626', name: 'Health & Family Welfare (7,595 Qs)' }},
    'Agriculture and Farmers Welfare': {{ x: width * 0.50, y: height * 0.22, color: '#16A34A', name: 'Agriculture & Farmers (5,122 Qs)' }},
    'Education': {{ x: width * 0.78, y: height * 0.25, color: '#2563EB', name: 'Education (3,589 Qs)' }},
    'Environment, Forest and Climate Change': {{ x: width * 0.22, y: height * 0.56, color: '#0D9488', name: 'Environment & Climate (3,535 Qs)' }},
    'Rural Development': {{ x: width * 0.50, y: height * 0.54, color: '#7C3AED', name: 'Rural Development (2,545 Qs)' }},
    'Women and Child Development': {{ x: width * 0.78, y: height * 0.56, color: '#D946EF', name: 'Women & Child Dev (3,063 Qs)' }},
    'Labour and Employment': {{ x: width * 0.35, y: height * 0.80, color: '#D97706', name: 'Labour & Employment (2,688 Qs)' }},
    'Other': {{ x: width * 0.68, y: height * 0.80, color: '#8C8780', name: 'Other Ministries' }}
  }};

  function getClusterTarget(d) {{
    if (d.top_ministries && d.top_ministries.length) {{
      for (let item of d.top_ministries) {{
        if (clusters[item.m]) return clusters[item.m];
      }}
    }}
  ...
  Object.values(clusters).forEach(c => {{
    const labelGroup = gAnnotations.append('g')
      .attr('transform', `translate(${{c.x}}, ${{c.y - 46}})`);

    // Pill background
    const bgRect = labelGroup.append('rect')
      .attr('rx', 5)
      .attr('fill', 'rgba(250, 247, 242, 0.96)')
      .attr('stroke', c.color)
    ...
    const txtWidth = txt.node().getComputedTextLength() + 18;
  ...
  simulation
    .force('x', d3.forceX(d => getClusterTarget(d).x).strength(0.55))
    .force('y', d3.forceY(d => getClusterTarget(d).y).strength(0.55))
    .force('collide', d3.forceCollide(d => d.radius + 1.2).strength(0.75))
    .alpha(0.45).restart();

  updateNodeAppearance(
    d => getClusterTarget(d).color,
    () => 0.85,
    () => false,
    () => 4.8
  );
